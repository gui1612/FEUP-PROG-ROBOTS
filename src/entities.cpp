...
void updateMaze(Maze &maze, Robot &robot, Player &player, char key, vector robotIDS) {
    ...
    for (int i; i < robotIDS.size(); i++) {
        int x = robot.id.coordinates(0);
        int y = robot.id.coordinates(1);

        maze.coordinates.at(y).at(x) = ' ';
    }

    int x1 = player.coordinates.at(0);
    int y1 = player.coordinates.at(1);

    maze.coordinates.at(y).at(x) = ' ';

    updatePlayer(player, key, maze);
...
void updatePlayer(Player &player, char key, Maze &maze){
    ...
    Point lastPos = {player.coordinates.at(0), player.coordinates.at(1)};
    ...
    // New position
    xval x = player.coordinates.at(0);
    yval y = player.coordinates.at(1);

    if (maze.coordinates.at(y).at(x) == '*' || maze.coordinates.at(y).at(x) == 'R'){    // Player has died
        player.alive = false;
    } else {                                                                            // Player has not died
        playerDraw(lastPos, player, maze);
    }
}
...
void playerDraw(Point lastPos, Player player, Maze &maze) {
    ...
    xval x = player.coordinates.at(0);
    yval y = player.coordinates.at(1);
    maze.coordinates.at(y).at(x) = 'H';                     // Replacing player new position
}
